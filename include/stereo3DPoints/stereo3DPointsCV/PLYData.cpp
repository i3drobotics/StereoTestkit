...
filename(outputfile), format(outputformat), data(outputData)
{
    ...
    if (outputColor.channels() == 3)
    {
        ...
        cv::cvtColor(outputColor, colors, CV_GRAY2BGR);
    }
    ...
    /*CV_Assert(
              ((data.cols == 1 && data.type() == CV_32FC1) || (data.cols == 3 && data.type() == CV_32FC3)) &&
...
void DataExporter::exportToFile() {
    ...
    "element vertex " << valid_count << std::endl <<
    ...
    const unsigned char* pColor = colors.ptr<unsigned char>(0);
                               // Number of iteration (3 channels * numElem)
    ...
    switch (format) {
        ...
        for (unsigned long i = 0; i<numIter; i+=3) {
        ...
        case FileFormat::PLY_ASCII:
            for (unsigned long i = 0; i<numIter; i+=3) {                           // Loop through all elements
                if (isValidPoint(pData[i+2])){
                    for (unsigned int j = 0; j<3; j++) {                                // Loop through 3 coordinates
                        float v = pData[i+j];
                        filestream << std::setprecision(9) << v << " ";
                    }
                    for (int j = 2; j>=0; j--) {
                        // OpenCV uses BGR format, so the order of writing is reverse to comply with the RGB format
...
bool DataExporter::isValidPoint(float v)
{
  // Check both for disparities explicitly marked as invalid (where OpenCV maps pt.z to MISSING_Z)
  // and zero disparities (point mapped to infinity).
  float missing_z_val = 10000;
  if (v == missing_z_val || std::isinf(v) || std::isnan(v)){
      return false;
  } else {
      return true;
  }
  //return pt[2] != missing_z_val && !std::isinf(pt[2]);
}
